  ...
  return (
    <>
      <div className='flex-col lg:my-4 xs:m-0 absolute top-0 left-0 bg-white h-full w-full overflow-hidden [z-index:0]' >
        <OrderList unstyled className='h-full w-full lg:left-auto [z-index:2]' {...{ value: items, itemTemplate, header: `Shopping List (${items.length} item${items.length !== 1 ? 's' : ''})`, onChange: e => setItems(e.value) }} dragdrop={true} pt={{ header: { className: 'fixed top-[10%] left-auto w-full xs:h-[10%] lg:w-6/12 lg:left-auto mx-auto [z-index:30] !bg-white/30 backdrop-blur-md text-black p-2 font-bold text-3xl w-full border-b-gray-400 border-style-solid border-2 m-0 p-2' }, list: { className: 'sticky !top-[10%] left-0 w-full xs:!h-[90%] sm:!h-[70%] [z-index:15]  ' }, root: { className: 'relative h-full w-full [z-index:5]' }, container: { className: 'absolute top-[10%] !h-[85%] w-full [z-index:10]' }, controls: { className: 'hidden' } }} />
